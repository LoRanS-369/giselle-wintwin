...
import { Capability, hasCapability, languageModels } from "@giselles-ai/language-model"
import type {
  CompletedGeneration,
  FailedGeneration,
  GenerationOutput,
...
export function generateContent({
  ...
  return useGenerationExecutor({
    ...
    execute: async ({
      ...
      const languageModel = languageModels.find(lm => lm.id === operationNode.content.llm.id)
      if (!languageModel) {
        ...
        for (const tool of postgresToolData.tools) {
          if (tool in postgresTool.toolSet) {
            preparedToolSet = {
              ...
              toolSet: {
                ...preparedToolSet.toolSet,
                [tool]: postgresTool.toolSet[tool as keyof typeof postgresTool.toolSet],
              },
            }
          }
          preparedToolSet = {
            ...preparedToolSet,
            cleanupFunctions: [...preparedToolSet.cleanupFunctions, postgresTool.cleanup],
          }
        }
      }
      ...
      if (
        ...
        preparedToolSet = {
          ...
          toolSet: {
            ...preparedToolSet.toolSet,
            web_search: openai.tools.webSearch(operationNode.content.tools.openaiWebSearch),
          },
        }
      }
      if (
      ...
      const model = generationModel(operationNode.content.llm, aiGatewayHeaders)
      ...
      const streamTextResult = streamText({
        ...
        onFinish: () => {
          logger.info(`Text generation completed in ${Date.now() - textGenerationStartTime}ms`)
        },
      ...
      const uiMessageStream = streamTextResult.toUIMessageStream({
        onFinish: async ({ messages: generateMessages }) => {
          ...
          await Promise.all(
            preparedToolSet.cleanupFunctions.map(cleanupFunction => cleanupFunction()),
          )
          logger.info(`Tool cleanup completed in ${Date.now() - toolCleanupStartTime}ms`)
          if (generationError) {
          ...
          logger.info(`Text retrieval completed in ${Date.now() - textRetrievalStartTime}ms`)
          ...
          logger.info(`Source retrieval completed in ${Date.now() - sourceRetrievalStartTime}ms`)
      ...
      const writer = batchWriter<StreamItem<typeof uiMessageStream>>({
        process: batch => {
          ...
          return context.storage.setBlob(
            ...
            new TextEncoder().encode(batch.map(chunk => JSON.stringify(chunk)).join("\n")),
          )
        },
        preserveItems: true,
        logger,
...
function getProviderOptions(languageModelData: TextGenerationLanguageModelData):
  ...
  | undefined {
  const languageModel = languageModels.find(model => model.id === languageModelData.id)
  if (
    languageModel &&
  ...
  if (languageModel && languageModelData.provider === "openai") {
    ...
    if (hasCapability(languageModel, Capability.Reasoning)) {
      openaiOptions.textVerbosity = languageModelData.configurations.textVerbosity
      openaiOptions.reasoningSummary = "auto"
      openaiOptions.reasoningEffort = languageModelData.configurations.reasoningEffort
    }
...
function generationModel(
  ...
  switch (llmProvider) {
    ...
    case "openrouter": {
      ...
      const modelId = languageModel.id.replace("openrouter/", "")
...
function generateContentV2({
  ...
  return useGenerationExecutor({
    ...
    execute: async ({
      ...
      const callOptions = transformGiselleLanguageModelToAiSdkLanguageModelCallOptions(
      ...
      const uiMessageStream = streamTextResult.toUIMessageStream({
        onFinish: async ({ messages: generateMessages }) => {
          ...
          const toolCleanupStartTime = Date.now()
          await Promise.all(cleanupFunctions.map(cleanupFunction => cleanupFunction()))
          logger.info(`Tool cleanup completed in ${Date.now() - toolCleanupStartTime}ms`)
          if (generationError) {
            if (AISDKError.isInstance(generationError)) {
              logger.error(generationError, `${generation.id} is failed`)
